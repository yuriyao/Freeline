...
public class LoginServlet extends HttpServlet {
    ...
    public void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        if(userService.login(userName, pwd)) {
            ...
            response.getWriter().println("<html><meta http-equiv=\"Content-Type\" content=\"text/html; charset=UTF-8\"> <title>成功</title></html>");
            response.getWriter().close();
        } else {
            logger.debug(userName + "登陆失败");
            response.getWriter().println("<html><meta http-equiv=\"Content-Type\" content=\"text/html; charset=UTF-8\"><title>失败</title></html>");
            response.getWriter().close();
        }
    }
}
